        ...
        <>
            <div>
                <div className='flex gap-x-8'>
                    <div>
                        <div className='pl-2 pr-2 hover:cursor-pointer' onClick={() => { changeClassification(false) }}>Dành cho bạn</div>
                        <div className={`${!classification ? 'bg-blue-500' : ''} h-1 w-full`}></div>
                    </div>
                    <div>
                        <div className='pl-2 pr-2 hover:cursor-pointer' onClick={() => { changeClassification(true) }}>Đánh giá cao nhất</div>
                        <div className={`${classification ? 'bg-blue-500' : ''} h-1 w-full`}></div>
                    </div>
                </div>
                <hr className='w-[65%]' />
                {data.map((p) => {
                    return <div key={p.id} className='flex gap-x-4 mt-2 mb-2 bg-gray-300'>
